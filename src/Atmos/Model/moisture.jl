...
@inline function atmos_nodal_update_aux!(moist::EquilMoist, atmos::AtmosModel,
  ...
  TS = PhaseEquil(e_int, state.ρ, state.moisture.ρq_tot/state.ρ, moist.maxiter, FT(100))
  #TS = PhaseEquil(e_int, state.ρ, state.moisture.ρq_tot/state.ρ, moist.maxiter, FT(1e-1), MoistThermodynamics.saturation_adjustment_SecantMethod)
