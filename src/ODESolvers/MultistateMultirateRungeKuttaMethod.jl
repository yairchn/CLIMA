...
using CLIMA.DGmethods: initialize_fast_state!,
...
function ODEs.dostep!(Qvec, msmrrk::MSMRRK{SS}, param,
                      time::Real, slow_dt::AbstractFloat
                     ) where {SS <: LSRK2N}
  ...
  for slow_s = 1:length(slow.RKA)
    ...
    @launch(device(Q), threads=threads, blocks=blocks,
            update!(realview(dQslow), realview(Qslow), RKA[s%length(RKA)+1],
                    RKB[s], slow_dt))
    ...
    pass_tendency_from_slow_to_fast!(slow_bl, fast_bl, slow.rhs!, fast.rhs!, Qfast, realview(dQ2fast))
...
#  ---> need to cumulate U at this time (and not at each RKB sub-time-step)
      cummulate_fast_solution!(fast_bl, Qfast, fast_time, fast_dt, total_fast_step)
    end

    ### later testing ignore this
    # reconcile slow equation using fast equation
    reconcile_from_fast_to_slow!(slow_bl, fast_bl, slow.rhs!, slow.fast!,
                                 realview(dQslow), realview(Qslow), realview(Qfast),
                                 total_fast_step)
  end
  return nothing
end
