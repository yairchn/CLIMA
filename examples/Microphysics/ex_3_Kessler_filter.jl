...
function main(mpicomm, DFloat, topl::AbstractTopology{dim}, N, timeend,
  ...
  cbvtk = GenericCallbacks.EveryXSimulationSteps(120) do (init=false)
    ...
    outprefix = @sprintf("vtk/ex_3_filter_%dD_mpirank%04d_step%04d",
  ...
  cb_filter = GenericCallbacks.EveryXSimulationSteps(1) do
    DGBalanceLawDiscretizations.apply!(Q, 7:_nstate, spacedisc,
                                       filter;
                                       horizontal=true,
                                       vertical=true)
    nothing
  ...
  solve!(Q, lsrk; timeend=timeend, callbacks=(cbinfo, cbvtk, cb_filter))
