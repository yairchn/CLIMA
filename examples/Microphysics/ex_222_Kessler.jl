...
# time tendencies
@inline function source!(Sin, Sout, Q, aux, t)
  u, w, rain_w, ρ, q_tot, q_liq, q_rai, e_tot = preflux(Q)
  @inbounds begin
    DF = eltype(Q)
    ...
    src_ρq_liq = ρ * conv_q_vap_to_q_liq(q_eq, q) # TODO - ensure positive definite
    # no pair?
    if src_ρq_liq > 0
      Sin[_ρq_liq] += src_ρq_liq
    else
      Sout[_ρq_liq] += src_ρq_liq
    end
    ...
    if(q_tot >= DF(0) && q_liq >= DF(0) && q_rai >= DF(0))
      ...
      src_ρq_rai_acnv = ρ * conv_q_liq_to_q_rai_acnv(q.liq)
      src_ρq_rai_accr = ρ * conv_q_liq_to_q_rai_accr(q.liq, q_rai, ρ)
      src_ρq_rai_evap = ρ * conv_q_rai_to_q_vap(q_rai, q, T , p, ρ)

      α = (DF(e_int_v0) - (DF(cv_v) - DF(cv_d)) * (T - DF(T_0)))

      if src_ρq_rai_acnv > 0
        Sout[_ρq_liq] -= src_ρq_rai_acnv
        Sin[_ρq_rai]  += src_ρq_rai_acnv
        Sout[_ρq_tot] -= src_ρq_rai_acnv
        Sout[_ρe_tot] -= src_ρq_rai_acnv * α
      else
        Sin[_ρq_liq]  -= src_ρq_rai_acnv
        Sout[_ρq_rai] += src_ρq_rai_acnv
        Sin[_ρq_tot]  -= src_ρq_rai_acnv
        Sin[_ρe_tot]  -= src_ρq_rai_acnv * α
      end

      if src_ρq_rai_accr > 0
        Sout[_ρq_liq] -= src_ρq_rai_accr
        Sin[_ρq_rai]  += src_ρq_rai_accr
        Sout[_ρq_tot] -= src_ρq_rai_accr
        Sout[_ρe_tot] -= src_ρq_rai_accr * α
      else
        Sin[_ρq_liq]  -= src_ρq_rai_accr
        Sout[_ρq_rai] += src_ρq_rai_accr
        Sin[_ρq_tot]  -= src_ρq_rai_accr
        Sin[_ρe_tot]  -= src_ρq_rai_accr * α
      end

      if src_ρq_rai_evap > 0
        Sin[_ρq_rai]  += src_ρq_rai_evap
        Sout[_ρq_tot] -= src_ρq_rai_evap
        Sout[_ρe_tot] -= src_ρq_rai_evap * α
      else
        Sout[_ρq_rai] += src_ρq_rai_evap
        Sin[_ρq_tot]  -= src_ρq_rai_evap
        Sin[_ρe_tot]  -= src_ρq_rai_evap * α
      end
    end
  end
end
...
function main(mpicomm, DFloat, topl::AbstractTopology{dim}, N, timeend,
  ...
  cbvtk = GenericCallbacks.EveryXSimulationSteps(120) do (init=false)
    ...
    outprefix = @sprintf("vtk/ex_222_microphysics_Kessler_%dD_mpirank%04d_step%04d",
