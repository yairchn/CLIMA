...
Base.@kwdef struct SoilModelMoisture{Fκ, Fiθ, Fsθ} <: BalanceLaw
  ...
  surfaceθ::Fsθ = (state, aux, t) -> 0.6 #267 # [m3/m3] constant flux at surface, from Bonan, Ch.8, fig 8.8 as in Haverkamp et al. 1977, p.287
end
...
#   `D` Diffusion tensor
vars_aux(::SoilModelMoisture, Fθ) = @vars(z::Fθ, h::Fθ) # p::Fθ stored in dg.auxstate
vars_state(::SoilModelMoisture, Fθ) = @vars(θ::Fθ) # stored in Q
...
function flux_diffusive!(m::SoilModelMoisture, flux::Grad, state::Vars, diffusive::Vars, aux::Vars, t::Real)
   flux.θ -= m.κ(state, aux, t) * diffusive.∇h
   if aux.z == 0
   end
end
...
function init_aux!(m::SoilModelMoisture, aux::Vars, geom::LocalGeometry)
  aux.z = geom.coord[3]
  aux.h = 0.5 #aux.z+m.initialθ(state, aux, t) #^(-1/0.378))*(-0.3020)
end
