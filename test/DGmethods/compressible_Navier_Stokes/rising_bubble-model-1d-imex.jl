...
function run(mpicomm, LinearType,
  ...
  model = AtmosModel(FlatOrientation(),
                     ...
                     HydrostaticState(IsentropicProfile(FT(θ_ref)),FT(0)),
                     Vreman{FT}(C_smag),
                     EquilMoist(),
                     NoRadiation(),
                     NoSubsidence{FT}(),
  ...
  ark = ARK2GiraldoKellyConstantinescu(dg, lindg, linearsolver, Q; dt = dt, t0 = 0)
  ...
  step = [0]
  outputtime = 25
  outdir = "vtk-rtb-1d-imex"
  cbvtk = GenericCallbacks.EveryXSimulationSteps(floor(outputtime / dt))  do (init=false)
    mkpath(outdir)
      outprefix = @sprintf("%s/DC_%dD_mpirank%04d_step%04d", outdir, dim,
  ...
  solve!(Q, ark; timeend=timeend, adjustfinalstep=false, callbacks=(cbinfo,cbvtk))
