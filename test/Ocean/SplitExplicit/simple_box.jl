...
using CLIMA.MultistateMultirateRungeKuttaMethod
...
import CLIMA.DGmethods: update_aux!, update_aux_diffusive!, vars_state, vars_aux, VerticalDirection
using GPUifyLoops
...
@inline function ocean_boundary_state!(m::Union{OceanModel, HorizontalModel}, p::SimpleBox, bctype, x...)
  if bctype == 1
...
function main()
  ...
  grid_2D = DiscontinuousSpectralElementGrid(topl_2D,
                                             FloatType = FT,
                                             DeviceArray = ArrayType,
                                             polynomialorder = N)

  brickrange_3D = (xrange, yrange, zrange)
  topl_3D = StackedBrickTopology(mpicomm, brickrange_3D;
                                 periodicity = (false, false, false),
                                 boundary = ((1, 1), (1, 1), (2, 3)))
  grid_3D = DiscontinuousSpectralElementGrid(topl_3D,
                                             FloatType = FT,
                                             DeviceArray = ArrayType,
                                             polynomialorder = N)
  ...
  @info @sprintf("""Update
      ...
      @info @sprintf("""Update
                    ...
                    Diffusive CFL = %.1f
                    Timestep      = %.1f""",
                 CFL_viscous, CFL_diffusive, dt_slow)
  ...
  horizontal_dg = DGModel(horizontalmodel,
                         grid_3D,
                         Rusanov(),
                         CentralNumericalFluxDiffusive(),
                         CentralNumericalFluxGradient();
  ...
  barotropic_dg = DGModel(barotropicmodel,
                         grid_2D,
                         Rusanov(),
                         CentralNumericalFluxDiffusive(),
                         CentralNumericalFluxGradient())

  Q_3D = init_ode_state(dg, FT(0); forcecpu=true)
  update_aux!(dg, model, Q_3D, FT(0))
  update_aux_diffusive!(dg, model, Q_3D, FT(0))

  Q_2D = init_ode_state(barotropic_dg, FT(0); forcecpu=true)

  lsrk_ocean = LSRK144NiegemannDiehlBusch(dg, Q_3D, dt = dt_slow, t0 = 0)
  lsrk_horizontal = LSRK144NiegemannDiehlBusch(horizontal_dg, Q_3D, dt = dt_slow, t0 = 0)
  lsrk_barotropic = LSRK144NiegemannDiehlBusch(barotropic_dg, Q_2D, dt = dt_fast, t0 = 0)

  odesolver = MultistateMultirateRungeKutta(lsrk_ocean, lsrk_horizontal, lsrk_barotropic)
