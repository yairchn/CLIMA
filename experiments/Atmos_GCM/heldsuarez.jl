...
function config_heldsuarez(FT, poly_order, resolution)
  ...
  temp_profile_ref  = IsothermalProfile(T_sfc) 
  ...
  model = AtmosModel{FT}(
    ...
    moisture    = DryModel(),
    source      = (),# Coriolis()),#, held_suarez_forcing!, sponge),
...
function main()
  ...
  #ode_solver_type = CLIMA.ExplicitSolverType(solver_method=LSRK54CarpenterKennedy)
