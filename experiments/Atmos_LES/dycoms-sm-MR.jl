...
function flux_radiation!(m::DYCOMSRadiation, atmos::AtmosModel, flux::Grad, state::Vars,
  ...
  free_troposphere_flux = m.ρ_i * FT(cp_d) * D_subs * m.α_z * cbrt(Δz_i) * (Δz_i/4 + m.z_i)
...
function config_dycoms(FT, N, resolution, xmax, ymax, zmax)
    ...
    model = AtmosModel{FT}(AtmosLESConfiguration;
                           ...
                           moisture=EquilMoist(maxiter=max_moist_iter, tolerance=tol),
                           radiation=radiation,
    ...
    #EXPL with LSRK144NiegemannDiehlBusch
    config = CLIMA.LES_Configuration("DYCOMS", N, resolution, xmax, ymax, zmax,
                                     init_dycoms!,
                                     solver_type=CLIMA.ExplicitSolverType(solver_method=MultirateRungeKutta),
                                     model=model)
    #IMEX by default:
    #=config = CLIMA.LES_Configuration("DYCOMS", N, resolution, xmax, ymax, zmax,
                                     init_dycoms!,
...
function main()
    ...
    solver_config = CLIMA.setup_solver(t0, timeend, driver_config, Courant_number=0.4, forcecpu=true)
